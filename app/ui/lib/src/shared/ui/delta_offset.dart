...
class DeltaOffset extends Offset {
  /// Creates a [DeltaOffset] based on right delta.
  const DeltaOffset.right(
    double right,
  ) : super(right, 0.0);

  /// Creates a [DeltaOffset] based on top delta.
  const DeltaOffset.top(
    double top,
  ) : super(0.0, top * -1);

  /// Creates a [DeltaOffset] based on left delta.
  const DeltaOffset.left(
    double left,
  ) : super(left * -1, 0.0);

  /// Creates a [DeltaOffset] based on bottom delta.
